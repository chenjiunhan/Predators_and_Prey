            ...
            while count_evalution < NUM_EVALUATION and count_evalution <= len(self.prey[0].best_nets): 
                ...
                if count_evalution == 0 or len(self.prey[0].best_nets) == 0:
                    ...
                    if prey_bool:
                        ...
                        for i in range(NUM_PREDATORS):                            
                            play_net += [random.choice(self.predators[i].best_nets)]
                    ...
                    if type(info) != type(None) and 'time' in info and not EVALUATION:
                        ...
                        if self.target.name != 'prey':                    
                            ...
                            if self.target.name == 'predator0':
                                if (info["time"] > 3 and s0_avg / step < 0.002) or (info["time"] > 3 and look_at_fitness / step < 0.2):
                                    predator0_break = True
                                    break
                                    
                            if self.target.name == 'predator1':
                                if (info["time"] > 3 and s1_avg / step < 0.002) or (info["time"] > 3 and look_at_fitness / step < 0.2):
                                    predator1_break = True
                                    break
                                    
                            if self.target.name == 'predator2':
                                if (info["time"] > 3 and s2_avg / step < 0.002) or (info["time"] > 3 and look_at_fitness / step < 0.2):
