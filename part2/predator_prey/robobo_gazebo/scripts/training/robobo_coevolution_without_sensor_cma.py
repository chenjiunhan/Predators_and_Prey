...
class DummyPlayer():
    ...
    def __call__(self, x):
        return torch.from_numpy(np.array([0.0, 0.0]))
    def activate(self, x):
        return np.array([0.0, 0.0])

class EvolvedAgent:
    
    def __init__(self, agent_type, name, ea_framework, idx, algorithm):
        ...
        self.cma_elitism = 4
        self.cma_pop_size = 20
        self.count_individual = 0
        self.generation = -1
        ...
        elif self.algorithm == "CMA":
            if self.type == 0:            
                net = Net(PREDATOR_INPUT_DIM, 2)
            else:    
                net = Net(PREY_INPUT_DIM, 2)
            
            params_list = self.tensors_to_params_list(list(net.parameters()))


            opts = cma.CMAOptions()
            opts.set('popsize', self.cma_pop_size)
            opts.set('CMA_elitist', 'initial')
            self.es = cma.CMAEvolutionStrategy(params_list, 0.5, opts)            
    ...
    def tensors_to_params_list(self, tensors):    
    ...
    def agent_thread(self):
        ...
        if self.algorithm == "NEAT":
            ...
            self.es.optimize(self.cma_objective_function)
            
    def params_list_into_net(self, params_list, net):
    ...
    def cma_objective_function(self, params_list):
        
        
        print("count indi mod:", self.count_individual % self.cma_pop_size)

        if self.count_individual % self.cma_pop_size == 0:
            ...
            if self.generation > 0:
                ...
                data_dict = {'nets': self.prev_nets, 'fitnesses': self.prev_fitnesses, 'generation': self.generation - 1}
                output = open('generations/' + self.name + "/" + str(self.generation - 1) + '.pkl', 'wb')
                pickle.dump(data_dict, output)
            
            print("GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG", self.generation)
        ...
        if self.type == 0:            
            new_net = Net(PREDATOR_INPUT_DIM, 2)
        else:    
            new_net = Net(PREY_INPUT_DIM, 2)
            
        self.params_list_into_net(params_list, new_net)                                           
        
        self.nets += [new_net]  
        
        if DEBUG_WEIGHT:
            print("DEBUG_WEIGHT")
            print(new_net.fc1.weight, new_net.fc1.bias)
            print(new_net.fc3.weight, new_net.fc3.bias)
        ...
        fitness = copy.deepcopy(self.fitnesses[-1])
        
        if DEBUG_FITNESS:
            print("Fitness CMA:", fitness, self.name)
...
class EAframework:
    ...
    def __init__(self, num_predators, num_prey):
        ...
        for i in range(num_predators):       
            self.predators += [EvolvedAgent(0, "predator" + str(i), self, len(self.predators), ALGORITHM)];
            
        for i in range(num_prey):
            self.prey += [EvolvedAgent(1, "prey", self, len(self.prey), ALGORITHM)];                            
    ...
    def start_evolve(self):        
        ...
        while not rospy.is_shutdown():            
            ...
            while not done and not rospy.is_shutdown():
                ...
                obs0 = np.array([d0p, y0p], dtype=np.double)
                obs1 = np.array([d1p, y1p], dtype=np.double)
                obs2 = np.array([d2p, y2p], dtype=np.double)
                obsp = np.array([d0p, d1p, d2p, yp0, yp1, yp2], dtype=np.double)
                ...
                if DEBUG_MANUAL:
                    if self.predators[0].algorithm == "NEAT":
                        ...
                        action[0,:] = play_net[0](torch.from_numpy(obs0).type(torch.DoubleTensor)).detach().numpy() * 0
                        action[1,:] = play_net[1](torch.from_numpy(obs1).type(torch.DoubleTensor)).detach().numpy() * 0
                        action[2,:] = play_net[2](torch.from_numpy(obs2).type(torch.DoubleTensor)).detach().numpy() * 0
                        action[3,:] = play_net[3](torch.from_numpy(obsp).type(torch.DoubleTensor)).detach().numpy() * 0
                else:
                    if self.predators[0].algorithm == "NEAT":
                        ...
                        #print(play_net[0])
                        #print(torch.from_numpy(obs0))
                        action[0,:] = play_net[0](torch.from_numpy(obs0).type(torch.DoubleTensor)).detach().numpy() * MAX_SPEED
                        action[1,:] = play_net[1](torch.from_numpy(obs1).type(torch.DoubleTensor)).detach().numpy() * MAX_SPEED
                        action[2,:] = play_net[2](torch.from_numpy(obs2).type(torch.DoubleTensor)).detach().numpy() * MAX_SPEED
                        action[3,:] = play_net[3](torch.from_numpy(obsp).type(torch.DoubleTensor)).detach().numpy() * MAX_SPEED
